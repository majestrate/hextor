$SIG{__WARN__} = sub {
	...
	if( $message =~ /^(?:Gtk|GLib|Gdk)(?:-\w+)?-$message_levels/i ) {
		...
		if( defined &Hextor::Internal::print ) {
			Hextor::print( $message );
		} else {
			warn $message;
		}
	}
...
sub get_context;
sub Hextor::Internal::context_info;
sub Hextor::Internal::print;
...
our %EXPORT_TAGS = (
	constants => [
		...
		qw(EAT_NONE EAT_HEXTOR EAT_XCHAT EAT_PLUGIN EAT_ALL), # callback return values
...
sub register {
	my ($package, $calling_package) = Hextor::Embed::find_pkg();
	my $pkg_info = Hextor::Embed::pkg_info( $package );
	my $filename = $pkg_info->{filename};
	my ($name, $version, $description, $callback) = @_;
	
	if( defined $pkg_info->{gui_entry} ) {
		Hextor::print( "Hextor::register called more than once in "
			. $pkg_info->{filename} );
		return ();
	}
	...
	if( $callback ) {
		$callback = Hextor::Embed::fix_callback(
	...
	$pkg_info->{gui_entry} =
		Hextor::Internal::register( $name, $version, $description, $filename );
...
sub hook_server {
	...
	my ($priority, $data) = ( Hextor::PRI_NORM, undef );
	...
	my $pkg_info = Hextor::Embed::pkg_info( $package );
	my $hook = Hextor::Internal::hook_server(
...
sub hook_command {
	...
	my ($priority, $help_text, $data) = ( Hextor::PRI_NORM, undef, undef );
	_process_hook_options(
		$options,
	...
	my $pkg_info = Hextor::Embed::pkg_info( $package );
	my $hook = Hextor::Internal::hook_command(
...
sub hook_print {
	...
	my ($priority, $run_after, $filter, $data) = ( Hextor::PRI_NORM, 0, 0, undef );
	_process_hook_options(
		$options,
	...
	if( $run_after and $filter ) {
		Carp::carp( "Hextor::hook_print's run_after_event and filter options are mutually exclusive, you can only use of them at a time per hook" );
		return;
	}
	...
	my $pkg_info = Hextor::Embed::pkg_info( $package );
	my $hook = Hextor::Internal::hook_print(
...
sub hook_timer {
	...
	my $pkg_info = Hextor::Embed::pkg_info( $package );
	my $hook = Hextor::Internal::hook_timer( $timeout, $callback, $data, $package );
	push @{$pkg_info->{hooks}}, $hook if defined $hook;
	return $hook;
}
...
sub hook_fd {
	...
	my ($flags, $data) = (Hextor::FD_READ, undef);
	_process_hook_options(
		$options,
		[qw(flags data)],
		[\($flags, $data)],
	...
	my $pkg_info = Hextor::Embed::pkg_info( $package );
	my $hook = Hextor::Internal::hook_fd(
		$fileno, $cb, $flags, {
...
sub unhook {
	...
	if( defined( $hook )
		...
		return Hextor::Internal::unhook( $hook );
	}
	return ();
}
...
sub _do_for_each {
	...
	my $num_done = 0;
	my $old_ctx = Hextor::get_context();
	for my $server ( @$servers ) {
		for my $channel ( @$channels ) {
			if( Hextor::set_context( $channel, $server ) ) {
				$cb->();
				$num_done++
			}
		}
	}
	Hextor::set_context( $old_ctx );
	return $num_done;
}
...
sub print {
	...
	return _do_for_each(
		sub { Hextor::Internal::print( $text ); },
		@_
	);
}
...
sub printf {
	my $format = shift;
	Hextor::print( sprintf( $format, @_ ) );
}

# make Hextor::prnt() and Hextor::prntf() as aliases for Hextor::print() and 
# Hextor::printf(), mainly useful when these functions are exported
sub prnt {
	goto &Hextor::print;
}
...
sub command {
	...
	return _do_for_each(
		sub { Hextor::Internal::command( $_ ) foreach @commands },
		@_
	);
}
...
sub commandf {
	my $format = shift;
	Hextor::command( sprintf( $format, @_ ) );
}
...
sub plugin_pref_set {
	...
	return Hextor::Internal::plugin_pref_set($setting, $value);
}

sub plugin_pref_get {
	my $setting = shift // return 0;

	return Hextor::Internal::plugin_pref_get($setting);
}

sub plugin_pref_delete {
	my $setting = shift // return 0;

	return Hextor::Internal::plugin_pref_delete($setting);
}

sub plugin_pref_list {
	my %list = Hextor::Internal::plugin_pref_list();
...
sub set_context {
	...
	if( @_ == 2 ) {
		my ($channel, $server) = @_;
		$context = Hextor::find_context( $channel, $server );
	} elsif( @_ == 1 ) {
		if( defined $_[0] && $_[0] =~ /^\d+$/ ) {
			$context = $_[0];
		} else {
			$context = Hextor::find_context( $_[0] );
		}
	} elsif( @_ == 0 ) {
		$context = Hextor::find_context();
	}
	return $context ? Hextor::Internal::set_context( $context ) : 0;
}
...
sub get_info {
	...
	if( defined( $id ) ) {
		if( grep { $id eq $_ } qw(state_cursor id) ) {
			$info = Hextor::get_prefs( $id );
		} else {
			$info = Hextor::Internal::get_info( $id );
		}
	}
	return $info;
}
...
sub user_info {
	my $nick = Hextor::strip_code(shift @_ || Hextor::get_info( "nick" ));
	my $user;
	for (Hextor::get_list( "users" ) ) {
		if ( Hextor::nickcmp( $_->{nick}, $nick ) == 0 ) {
			$user = $_;
			last;
		}
	}
...
sub context_info {
	my $ctx = shift @_ || Hextor::get_context;
	my $old_ctx = Hextor::get_context;
	my @fields = (
	...
	if( Hextor::set_context( $ctx ) ) {
		my %info;
		for my $field ( @fields ) {
			$info{$field} = Hextor::get_info( $field );
		}
		
		my $ctx_info = Hextor::Internal::context_info;
...
sub get_list {
	...
	if( $_[0] eq 'networks' ) {
		return Hextor::List::Network->get();
	} else {
		return Hextor::Internal::get_list( $_[0] );
	}
}
