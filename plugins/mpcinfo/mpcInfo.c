...
static int mpc_tell(char *word[], char *word_eol[], void *userdata){
       ...
       if (hwnd==0) {hextor_print(ph, randomLine(notRunTheme));return HEXTOR_EAT_ALL;}
	   ...
	   {
		   g_free(tTitle);
		   hextor_print(ph, "pattern not found");
		   return HEXTOR_EAT_ALL;
	   }

       if ((tTitle[1]==':')&&(tTitle[2]=='\\')){
          //hextor_print(ph,"seams to be full path");
          if (endsWith(tTitle,".mp3")==1){
             //hextor_print(ph,"seams to be a mp3 file");
             info = readHeader(tTitle);
             
             if ((info.artist!=NULL)&&(strcmp(info.artist,"")!=0)){
                char *mode=MODES[info.mode];
                //hextor_printf(ph,"mode: %s\n",mode);
				...
				g_free(tTitle);
				hextor_command(ph, mp3Line);
                return HEXTOR_EAT_ALL;
             }
          }
          if (endsWith(tTitle,".ogg")==1){
             hextor_printf(ph,"Ogg detected\n");
...
int hextor_plugin_init(hextor_plugin *plugin_handle, char **plugin_name, char **plugin_desc, char **plugin_version, char *arg){
	...
	*plugin_version=VERSION;
	...
	hextor_hook_command(ph, "mpc", HEXTOR_PRI_NORM, mpc_tell,"no help text", 0);
	hextor_hook_command(ph, "mpc_themes", HEXTOR_PRI_NORM, print_themes,"no help text", 0);
	hextor_hook_command(ph, "mpc_reloadthemes", HEXTOR_PRI_NORM, mpc_themeReload,"no help text", 0);
	hextor_command (ph, "MENU -ishare\\music.png ADD \"Window/Display Current Song (MPC)\" \"MPC\"");
...
{
	hextor_command (ph, "MENU DEL \"Window/Display Current Song (MPC)\"");
	hextor_print (ph, "mpcInfo plugin unloaded\n");
	return 1;
}
