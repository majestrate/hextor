...
int str2int(char *text)
{
	...
	for (i = 1; i <= strlen(text); i++)
	{
		...
		{
			hextor_printf(ph, "invalid char in string: %i", (int) text[strlen(text) - i]);
			return 255;
		}
...
//if (DEBUG==1) putlog("reading ID3V1");
	...
	f=fopen(file,"rb");
	if (f==NULL){
       hextor_print(ph,"file not found while trying to read id3v1");
	...
	for (i=0;i<128;i++) {
		c=fgetc(f);
		if (c==EOF) {hextor_printf(ph,"read ID3V1 failed\n");fclose(f);return ret;}
		tag[i]=(char)c;
	}
	fclose(f);
	//printf("tag readed: \n");
	...
	//printf("header: %s\n",id);
	if (strcmp(id,"TAG")!=0){hextor_printf(ph,"no id3 v1 found\n");return ret;}
	...
	val=(int)tmp[0];
	if (val<0)val+=256;
	//hextor_printf(ph, "tmp[0]=%i (%i)",val,tmp[0]);
	if ((val<148)&&(val>=0)) 
	...
	//hextor_printf(ph, "tmp: \"%s\" -> %i",tmp,tmp[0]);
	//hextor_printf(ph,"genre \"%s\"",ret.genre);
...
//if (DEBUG==1) putlog("reading id3v2");
	...
	f = fopen(file,"rb");
	//hextor_printf(ph,"file :%s",file);
	if (f==NULL)
	{
       hextor_print(ph,"file not found whilt trying to read ID3V2");
...
//if (DEBUG==1) putlog("reading header");
	...
	if (f==NULL)
	{
       hextor_print(ph,"file not found while trying to read mp3 header");
